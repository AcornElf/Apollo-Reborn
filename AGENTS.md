...
| `src/settings/ApolloActionMenuSettingsViewController.{h,m}` + `src/ApolloActionMenuLayout.{h,m}` | Interface → Action Menus: per-menu (feed / post / post-in-comments / comment) ••• item order + hidden set, with a pinned live mock of the menu. The model (contexts, item catalogue keyed by Apollo's Action kinds, saved layouts) is Foundation-only; `ApolloActionMenu.xm` applies it by permuting the sheet's Swift `actions` buffer in place and filtering/ranking the registered specs |
